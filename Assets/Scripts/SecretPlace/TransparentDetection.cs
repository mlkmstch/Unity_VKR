...
public class TransparentDetection : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.GetComponent<Player>())
        {
            if (spriteRenderer)
            {
                ...
                camMono.StartCoroutine(FadeRoutine(spriteRenderer, fadeTime, spriteRenderer.color.a, transparencyAmount));
    ...
    private IEnumerator FadeRoutine(SpriteRenderer spriteRenderer, float fadeTime, float startValue, float targeTransparency)
    {
        ...
        while (elapsedTime < fadeTime)
        {
            ...
            spriteRenderer.color = new Color(spriteRenderer.color.r, spriteRenderer.color.b, newAlpha);
            yield return null;
        }
    }
    ...
    private IEnumerator FadeRoutine(Tilemap tilemap, float fadeTime, float startValue, float targeTransparency)
    {
        ...
        while (elapsedTime < fadeTime)
        {
            elapsedTime += Time.deltaTime;
            float newAlpha = Mathf.Lerp(startValue, targeTransparency, elapsedTime / fadeTime);
            tilemap.color = new Color(tilemap.color.r, tilemap.color.b, newAlpha);
            yield return null;
        }
    }
}
